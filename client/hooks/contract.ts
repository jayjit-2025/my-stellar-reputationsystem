...
export const CONTRACT_ADDRESS =
  "CDNFLBFTPQFJHK5Y4IPMXYG545I7PLGYD7C74QKZQEVM57PFWBHPBVZI";
...
// ============================================================
// Reputation System — Contract Methods
// ============================================================

/**
 * Create a new reputation topic. Anyone can create topics - fully permissionless.
 * Calls: create_topic(topic: String)
 */
export async function createTopic(caller: string, topic: string) {
  return callContract("create_topic", [toScValString(topic)], caller, true);
}

/**
 * Give reputation to an address for a topic. Anyone can give reputation to anyone.
 * Calls: give_reputation(giver: Address, to: Address, topic: String, value: i128)
 */
...
/**
 * Get reputation score for an address on a specific topic (read-only).
 * Calls: get_reputation(target: Address, topic: String) -> i128
 */
export async function getReputation(target: string, topic: string, caller?: string) {
  return readContract(
    "get_reputation",
    [toScValAddress(target), toScValString(topic)],
    caller
  );
}

/**
 * Get all available reputation topics (read-only).
 * Calls: get_topics() -> Vec<String>
 */
export async function getTopics(caller?: string) {
  return readContract("get_topics", [], caller);
}
